...
- If enabling the memory feature after openJiuwen has already started, in the same directory as the .env file run `cp .env.xxxxx .env` (xxxxx is a random code generated by the container runtime for the instance that needs the memory feature; you can view it via `docker ps -a`). Add the embedding-related information to .env; after configuring, restart the openJiuwen platform to apply the settings:
