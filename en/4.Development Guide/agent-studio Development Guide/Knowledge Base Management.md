...
8. On the document parameters page, configure document parsing and indexing parameters, then click **Next**.
   ...
   **About document graph construction**: Building a document graph can improve retrieval quality, especially for complex queries involving entities and relationships. Enabling it will significantly increase index build time, and the build process calls an LLM to extract entities and relationships from documents, which consumes additional LLM tokens. Consider enabling it only when the benefit in retrieval quality outweighs the extra build time and token cost.
