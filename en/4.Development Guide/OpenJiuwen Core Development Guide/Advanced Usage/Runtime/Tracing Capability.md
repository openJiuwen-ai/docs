The Runtime provides a tracing module to make diagnostics convenient for users. During Workflow execution, the module automatically collects event information and sends it to users in real time as **streaming messages**, transparently presenting the system’s internal states to users. It also supports user-injected custom tracing information. openJiuwen provides the following two tracing capabilities:

* **Basic tracing capability**: In Workflow mode, Runtime tracing records the start and final completion states of the entire workflow. It also performs more fine-grained event collection throughout the Workflow execution, including:
...
* **Custom tracing capability**: The Runtime also provides standardized tracing information collection interfaces, allowing developers to inject and collect **custom tracing information**.
...
To facilitate the debugging of custom components, the Runtime provides interfaces that allow developers to inject custom tracing information into components.

Create a custom component `AddComponent` for addition operations, and call the Runtime’s `trace` and `trace_error` capabilities to record custom tracing information.
