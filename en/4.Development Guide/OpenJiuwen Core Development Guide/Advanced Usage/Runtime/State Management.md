...
During the conversation execution, state data is accessed uniformly through `Runtime`, supporting cross-component sharing mechanisms and effectively reducing the complexity of property passing. All state data is centrally stored in `Runtime` and can be read and written via standardized interfaces, mainly divided into two categories:
