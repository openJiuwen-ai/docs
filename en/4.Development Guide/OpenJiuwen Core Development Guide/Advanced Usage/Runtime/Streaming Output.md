In the openJiuwen framework, workflows and Agent runtimes are implemented by the `Runtime` core class. This class enables real-time streaming of data to the outside of workflows or Agents, allowing efficient, real-time data processing and delivery, thus meeting high-performance and low-latency requirements. `Runtime` supports the following two streaming scenarios:
...
async def run_workflow(think_mode: bool):
    ...
    # Build inputs and workflow runtime, then invoke the workflow
...
In Agent scenarios, streaming output is typically implemented via the Agent’s skill modules (such as LLM services, workflows, etc.), by calling the `Runtime` streaming interfaces to achieve real-time, efficient data transmission and processing. The following demonstrates how to use `Runtime`’s two streaming output interfaces to design and implement an Agent with streaming capabilities.
...
class CustomAgent(BaseAgent):
    ...
    async def stream(self, inputs: Dict) -> AsyncIterator[Any]:
        ...
        # Obtain the current runtime through AgentRuntime in the base class
        runtime = await self._runtime.pre_run(session_id=session_id)

        async def stream_process():
            try:
                # Call the mock model and pass in the runtime to record streaming data in real time
        ...
        # Create an async task to produce streaming data
