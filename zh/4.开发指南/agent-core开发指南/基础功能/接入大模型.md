...
openJiuwen 在 0.1.4 中通过 `openjiuwen.core.foundation.llm` 提供统一的**模型客户端 + 配置**体系，推荐的接入方式是：

- 使用 `ModelClientConfig` 描述「如何连接模型服务」（provider/api_base/api_key/SSL 等）；
- 使用 `ModelRequestConfig` 描述「这次调用用哪个模型 + 调用参数」（model/temperature/top_p 等）；
- 使用具体的 `ModelClient` 实现（目前内置 `OpenAIModelClient`），通过 `invoke/stream` 进行调用。

# 使用 ModelClient 方式调用模型

## 初始化模型客户端

以硅基流动（兼容 OpenAI 接口）的模型接入为例：
...
# 1. 配置客户端连接信息（provider/api_base/api_key 等）
model_client_config = ModelClientConfig(
    model_provider="siliconflow",                 # 模型提供商标识符
    api_base="your path to model service",        # 模型服务 URL
    api_key="sk-****************************",    # 鉴权 Token
...
model_request_config = ModelRequestConfig(
    model="your_model_name",     # 具体模型名称，由服务端支持的模型列表决定
    temperature=0.7,
    top_p=0.9,
    timeout=30,
)
...
以同步非流式 `invoke` 和异步流式 `astream` 为例，介绍调用模型方式及运行结果，其他方式请参见 API 接口文档 `openjiuwen.core.foundation.llm`。

### 同步非流式invoke

通过 `model.invoke` 一次性获取模型响应的批输出结果。定义 system 提示词和用户输入，咨询与天气相关问题，预期调用工具，示例代码如下：
...
async def astream():
    try:
        ...
        async for chunk in model.astream(messages=messages, tools=tools):
            print(chunk)
...
准备模型输入参见本章节[准备模型输入](#准备模型输入)模块内容。

## 调用模型

调用模型参见本章节[调用模型](#调用模型)模块内容，其中 `model_request_config.model` 为 `api_base` 服务地址支持的模型名称。
