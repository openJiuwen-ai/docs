...
工作流分支组件，继承自 [WorkflowComponent](base.md#class-openjiuwencoreworkflowcomponentsbaseworkflowcomponent)。将当前节点加入图时，会为该节点添加条件边；运行时由 [BranchRouter](branch_router.md#class-branchrouter) 根据当前会话状态依次求值各分支条件，并返回首个满足条件的分支对应的目标节点列表。
...
```python
>>> from openjiuwen.core.workflow import Workflow, BranchComponent, Start, End
