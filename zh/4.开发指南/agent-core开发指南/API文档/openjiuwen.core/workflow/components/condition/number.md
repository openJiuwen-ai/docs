...
内部实现类 `NumberConditionInSession` 与 `NumberCondition` 类似，但构造参数为固定 `int`（`limit: int`），不依赖输入解析。适用于循环次数在配置或会话中已确定的场景。若 `limit` 为 `None`，会在 `invoke` 时抛出 `NUMBER_CONDITION_ERROR`。循环次数还受会话环境变量中的最大循环次数限制（如 `LOOP_NUMBER_MAX_LIMIT_KEY`），超过时由 [LoopComponent](../flow/loop/loop_comp.md#class-loopcomponent) 等上层抛出异常。
