...
* **JiuWenBaseException**：异常，当配置错误或模型调用失败时抛出。
...
>>> 
>>> class MyCustomModelClient(BaseModelClient):
>>>     """自定义模型客户端实现"""
>>> 
...
>>>         # 自定义初始化逻辑
>>> 
>>>     async def invoke(self, messages, *, tools=None, temperature=None, **kwargs):
>>>         # 实现具体的调用逻辑
...
>>>         )
>>>         # 调用自定义 API...
>>>         return AssistantMessage(content="响应内容")
>>> 
>>>     async def stream(self, messages, **kwargs):
>>>         # 实现流式调用逻辑
>>>         yield AssistantMessageChunk(content="流式")
>>>         yield AssistantMessageChunk(content="响应")
```
...
消息块基类，继承自 `BaseMessage`，支持 `__add__` 合并内容（字符串或列表拼接）。
...
## class openjiuwen.core.foundation.llm.output_parsers.markdown_output_parser.MarkdownOutputParser

Markdown 输出解析器，继承自 `BaseOutputParser`。将 Markdown 文本解析为结构化内容（如 MarkdownContent、MarkdownElement 等），支持流式解析。

### async parse(llm_output: Union[str, AssistantMessage]) -> Optional[MarkdownContent]

解析 LLM 输出中的 Markdown 内容。
...
**MarkdownContent**，解析后的 Markdown 结构化内容，解析失败返回 `None`。

---

## class openjiuwen.core.foundation.llm.output_parsers.markdown_output_parser.MarkdownContent

Markdown 内容结构化表示数据类。

* **raw_content**(str)：原始 Markdown 文本。默认值：`""`。
* **elements**(List[MarkdownElement])：所有元素按原始顺序排列。默认值：`[]`。
* **headers**(List[Dict[str, str]])：标题列表。默认值：`[]`。
* **code_blocks**(List[Dict[str, str]])：代码块列表。默认值：`[]`。
* **links**(List[Dict[str, str]])：链接列表。默认值：`[]`。
* **images**(List[Dict[str, str]])：图片列表。默认值：`[]`。
* **tables**(List[str])：表格列表。默认值：`[]`。
* **lists**(List[str])：列表内容。默认值：`[]`。

---

## class openjiuwen.core.foundation.llm.output_parsers.markdown_output_parser.MarkdownElement

单个 Markdown 元素数据类。

* **type**(str)：元素类型。
* **content**(Dict[str, Any])：元素内容。
* **start_pos**(int)：在原始文本中的起始位置。
* **end_pos**(int)：在原始文本中的结束位置。
* **raw**(str)：原始文本。

---

## class openjiuwen.core.foundation.llm.output_parsers.markdown_output_parser.MarkdownElementType

Markdown 元素类型常量。

* **HEADER**：标题，值为 `"header"`。
* **CODE_BLOCK**：代码块，值为 `"code_block"`。
* **INLINE_CODE**：行内代码，值为 `"inline_code"`。
* **LINK**：链接，值为 `"link"`。
* **IMAGE**：图片，值为 `"image"`。
* **TABLE**：表格，值为 `"table"`。
* **LIST**：列表，值为 `"list"`。
