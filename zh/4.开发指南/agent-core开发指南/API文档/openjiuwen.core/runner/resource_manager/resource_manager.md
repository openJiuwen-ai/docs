...
JiuWenBaseException：openJiuwen异常基类，具体详细信息和解决方法，参见StatusCode。
...
>>> agent_group_provider: AgentGroupProvider = lambda: BaseGroup()
>>> 
>>> result = await resource_mgr.add_agent_group(card, agent_group_provider, tag=["analysis", "team"])
>>> if result.is_ok():
...     print(f"添加代理组成功: {result.unwrap()}")
```
...
>>> agent_provider = lambda: BaseAgent()
>>> 
...
>>> resource_mgr = ResourceMgr()
>>> agents = [
...     (AgentCard(id="agent1", name="代理1"), lambda: BaseAgent()),
...     (AgentCard(id="agent2", name="代理2"), lambda: BaseAgent())
... ]
...
```python
>>> workflows = [
...     (WorkflowCard(id="wf1", name="工作流1"), lambda: Workflow()),
...     (WorkflowCard(id="wf2", name="工作流2"), lambda: Workflow())
... ]
...
>>> 
>>> model_provider: ModelProvider = lambda: SomeModel()
...
```python
>>> models = [
...     ("gpt-3.5", lambda: GPTModel()),
...     ("claude-2", lambda: ClaudeModel())
... ]
