...
在一些复杂的工作流构建场景下，在创建 `Workflow` 工作流时，可指定工作流的配置信息。工作流的配置信息由 `WorkflowConfig` 表示，其包含 `card`（`WorkflowCard`）、`spec`（`WorkflowSpec`）等。构造时传入 `card=WorkflowCard(id=..., name=..., version=...)` 指定工作流 ID、名称、版本等信息。
...
```python
from openjiuwen.core.workflow import WorkflowComponent, Session
