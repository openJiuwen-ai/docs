...
使用自定义模型类时，请参考《[接入大模型](../基础功能/接入大模型.md)》中基于 `ModelClientConfig` / `ModelRequestConfig` 的调用方式进行封装：

- 在自定义 `Model` 或自定义 `ModelClient` 中，实现 `invoke/stream` 等方法，并遵循 `openjiuwen.core.foundation.llm` 的接口约定；
- 在业务代码中，通过传入自定义的 `model_provider`、`api_base`、`api_key` 等配置，创建对应的 `ModelClient` 或 `Model` 实例后，与框架中其他支持的模型保持一致的调用方式。
