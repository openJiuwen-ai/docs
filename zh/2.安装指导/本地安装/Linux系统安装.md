本指南介绍在 Linux 系统采用本地方式安装 openJiuwen
...
openJiuwen 的记忆及知识库功能需依赖 Milvus 支撑，若需要体验记忆及知识库功能，可参考 [如何启用记忆及知识库功能](#linux-memory) 完成 Milvus 的安装配置。若仅需快速部署并体验 openJiuwen 基础功能，可直接跳过本步骤。
...
* 请在 *.env* 文件中根据实际情况修改以下变量的值（勿覆盖其他变量）：
  ...
  变量说明可参考如下表格，如需启用记忆及知识库功能，请参考[如何启用记忆及知识库功能](#linux-memory)
...
* 在源码根目录下，运行以下命令启动后端服务，并耐心等待：
  ...
  > **说明**：若要配置插件需开启沙箱服务，可参考 [如何启用沙箱功能](#linux-sandbox) 完成沙箱服务配置。
...
* 体验记忆功能请点击 “向量模型”，找到 BGE-M3 模型。体验知识库功能请根据场景自行选择模型。
...
3. 参考 `sandbox_server/gateway/.env.example` 文件，在 `sandbox_server/gateway` 目录下创建 `.env` 文件，示例如下：
   ...
   如果需要启动 bwrap 沙箱，请将 `ENABLE_LINUX_SANDBOX` 设置为1，并修改 `sandbox_server/gateway/conf/config.yaml`，确保 Python 解释器和 Js 解释器以及相关依赖都包的路径都在 `mount` 配置中，以及 `PATH` 环境变量中包含了 Python 解释器和 Js 解释器所在路径。示例如下：
   ...
   最后启动沙箱网关服务，即运行 `sandbox_server/gateway/server.py` 脚本。
