本章节演示了如何基于openJiuwen快速开发自定义Agent。该示例构建的智能体能够调用大模型服务，并且识别大模型输出结果中可能包含的敏感词。通过示例，你将会了解到如下信息：

- 如何继承`AgentConfig`基类，实现自定义智能体的配置类。
- 如何重写`Agent`基类的初始化方法`__init__`，实现自定义智能体的初始化方法。
- 如何实现`Agent`基类定义的批执行抽象接口`invoke`，异步批执行自定义智能体的具体业务逻辑。
...
接下来，开发者需要实现自定义智能体的初始化方法，支持创建自定义智能体的对象。`Agent`基类的初始化方法为智能体提供了初始化运行时`Runtime`、配置管理`Config`的能力。除此之外，自定义智能体除了调用`Agent`基类的初始化方法，还需要根据大模型的配置信息、创建大模型调用对象，初始化配置的敏感词列表。具体示例如下：
...
```python
from jiuwen.core.utils.llm.messages import HumanMessage, SystemMessage
