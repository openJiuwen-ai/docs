...
def _create_prompt_template():
    system_prompt = \
        ("你是一个AI助手，在适当的时候调用合适的工具，帮助我完成任务！\n"  # 人设&任务目标
         "今天的日期为：{}\n"                                          # 当前日期
...
首先使用openJiuwen提供的`create_react_agent_config`方法快速创建天气查询的`ReActAgentConfig`对象，涵盖`ReActAgent`相关的配置参数信息，如天气查询插件的元数据信息、提示词定义及大模型配置信息等。示例代码如下：
...
接着使用openJiuwen提供的`create_react_agent`方法快速创建`ReActAgent`对象，包括天气查询助手配置和关联的插件对象，示例代码如下：

```python
from jiuwen.agent.react_agent import create_react_agent

react_agent = create_react_agent(
            agent_config=react_agent_config,  # 天气查询助手配置
            tools=[_create_tool()]  # ReActAgent关联的插件对象
        )
```

`create_react_agent`方法实际上调用了`ReActAgent`的初始化方法：
...
async def main():
    ...
    model_config = ReactAgent._create_model()
