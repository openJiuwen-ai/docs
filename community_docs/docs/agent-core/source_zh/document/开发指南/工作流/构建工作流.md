...
```python
result = await workflow.invoke({"query": "查询上海的天气"}, runtime)
print(f"{result}")
```
...
```python
{"output": {}, "responseContent": "结果:Check the weather in Shanghai on 2025-08-22"}
```
...
在`flow.stream`方法中，指定`stream_modes=[BaseStreamMode.CUSTOM]`，表示只输出用户自定义的流式数据：
