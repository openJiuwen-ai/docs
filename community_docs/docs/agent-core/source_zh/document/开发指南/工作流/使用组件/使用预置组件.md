...
如下配置表明`query`为必选字段，将`Start`组件添加到工作流时须提供该输入字段，`dialogueHistory`和`conversationHistory`为非必选字段，`dialogueHistory`提供了非必选字段的默认值为`["c", "d"]`，`conversationHistory`提供了非必选字段的默认值为`[]`：
...
conf = {
    "inputs": [
        ...
        {"id": "conversationHistory", "default_value": [], "required": False},
    ]
}
...
workflow.set_start_comp("s", start, 
    inputs_schema={"query": "${user_inputs.query}","conversationHistory": "${user_inputs.conversationHistory}"})
```
...
`End`组件是openJiuwen工作流的内置结束组件，该组件定义了工作流的输出点。将`End`组件添加到工作流时可根据input_schema格式化`End`组件的输入参数，创建`End`组件时可通过`conf`模板渲染工作流的输出信息。`End`组件实现了基类组件`ComponentExecutable`的三个能力 `invoke`, `stream`和`transform`，实现的原理是对不同的输入进行相应的格式化输出。

- **invoke**：同步执行组件，一次性返回最终结果，适合一次性获取完整结果的场景，单次返回的最终结果中`responseContent` 表示支持模板渲染的渲染结果，`output`表示不支持模板渲染场景下`End`的输出结果。
  ...
  # 调用工作流
  result = await flow.invoke(inputs={"user_inputs": {"query": "你好", "content": "杭州"}},
                                          runtime=WorkflowRuntime())
  # 输出结果
  print(result.result)
  ```
...
- **stream**：`End`组件将批结果格式化为多帧openJiuwen`OutputSchema`格式的流式数据，适合需要实时反馈的场景，流式数据基于`payload`进行输出，支持渲染和非渲染情况。
  ...
  flow = Workflow()
  # 为工作流增加Start节点
  flow.set_start_comp("s", Start(), inputs_schema={"query": "${user_inputs.query}", "content":
              "${user_inputs.content}"})
  ```
  ...
  flow.add_connection("s", "e")
  # 调用工作流流式输出接口
  result = flow.stream(inputs={"user_inputs": {"query": "你好", "content": "杭州"}}, runtime=WorkflowRuntime(), stream_modes=[BaseStreamMode.OUTPUT])
  ```
  
  则`Start`组件输出即`End`组件输入为`{"query": "你好", "content": "杭州"}`，inputs_schema会格式化`End`组件输入为`{"param1": "你好", "param2": "杭州"}`，并输出到output字段。
...
- **transform**：`End`组件将流式输入逐帧格式化为`OutputSchema`格式的流式数据，`payload`为每个流式输入的内容，不支持渲染输出。
  ...
  ```python
  [{'type': 'PARTIAL_CONTENT', 'index': 0, 'payload': {'output': {'param1': '你好', 'param2': '杭州'}}}]
  ```
...
workflow_result = await workflow.invoke({"query": "时间是2025年8月1日", "conversation_id": "c123"}, workflow_runtime)
...
添加完条件判断逻辑，将分支组件添加到具有开始组件、自定义的绝对值计算组件和结束组件的工作流中：
...
```
When the user input num is 10, workflow run success, result is {'responseContent': '', 'output': {'raw': 10}}.

When the user input num is -10, workflow run success, result is {'responseContent': '', 'output': {'raw': -10, 'updated': 10}}.

When the user input num is 0, ALLOW_ZERO is true, workflow run success, result is {'responseContent': '', 'output': {'raw': 0, 'updated': 0}}.
...
- 当用户输入为10时，满足分支`pos_branch`，依次执行组件`start`，`branch`，`end`，工作流执行成功，输出结果为`{'responseContent': '', 'output': {'raw': 10}}`。
- 当用户输入为-10时，满足分支`neg_branch`，依次执行组件`start`，`branch`，`abs`，`end`，工作流执行成功，输出结果为`{'responseContent': '', 'output': {'raw': -10, 'updated': 10}}`。
- 当用户输入为0，环境变量`ALLOW_ZERO`为`true`时，满足`zero_branch`，依次执行组件`start`，`branch`，`abs`，`end`，工作流执行成功，输出结果为`{'responseContent': '', 'output': {'raw': 0, 'updated': 0}}`。
...
from jiuwen.core.component.loop_comp import LoopGroup
...
```python
from jiuwen.core.component.loop_comp import LoopComponent

# 创建LoopComponent，设置输出模式，捕获最终的中间变量、循环计数和循环结果
loop_component = LoopComponent(loop_group, {"user_var": "${l.user_var}", "loop_count": "${loop.index}", "results": "${a.output}"})
```
