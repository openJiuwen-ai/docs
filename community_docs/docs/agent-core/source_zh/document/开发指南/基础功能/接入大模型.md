...
- 厂商系列模型接入：全面支持硅基流动（SiliconFlow）厂商系列模型、遵循OpenAI接口系列模型的接入，用户可根据不同模型的特色（如推理能力、对话流畅性等）灵活选择适合业务场景的模型。通过`ModelFactory`提供了对大模型的统一调用接口。
...
> **说明**
> 用户需要自行前往硅基流动（SiliconFlow）或OpenAI的官网注册账号，以便获取模型广场中可用模型的`api_key`和模型调用的URL请求地址`api_base`。
...
以同步非流式invoke和异步流式astream为例，介绍调用模型方式及运行结果，其他方式请参见API接口文档`jiuwen.core.utils.llm`。
...
def test_invoke():
    ...
    # 调用模型
    response = model.invoke(model_name="Qwen/Qwen-32B", messages=messages, tools=tools, temperature=0.7, top_p=0.95)
...
```python
role='assistant' content='' name=None tool_calls=[ToolCall(args={}, id='019a11604a4a2a663d29e84dbbcb7e15', index=0, type='function', function=FunctionInfo(name='get_weather', arguments='{"location": "Hangzhou, China", "units": "metric"}'))] usage_metadata=UsageMetadata(code=0, errmsg='', prompt='', task_id='', model_name='Qwen/Qwen3-32B', finish_reason='tool_calls', total_latency=336.0, model_stats={}, first_token_time='', request_start_time='') raw_content=None reason_content=None
```
...
async def test_async_astream():
    try:
        ...
        # 使用async for遍历异步迭代器
        async for chunk in model.astream(model_name="Qwen/Qwen-32B", messages=messages, tools=tools, temperature=0.7,
                                         top_p=0.95):
            print(chunk.content)
...
async def main():
    ...
    await test_async_astream()
```

输出结果如下：

```python
type='assistant' content='你好' name=None tool_calls=ToolCall(name='', args={}, id='') usage_metadata=UsageMetadata(code=0, errmsg='成功', prompt='null', task_id=None, model_name='Qwen/QwQ-32B', finish_reason='null', total_latency=0, model_stats={}, first_token_time='', requests_start_time='') raw_content=None
type='assistant' content='！有什么我可以' name=None tool_calls=ToolCall(name='', args={}, id='') usage_metadata=UsageMetadata(code=0, errmsg='成功', prompt='null', task_id=None, model_name='Qwen/QwQ-32B', finish_reason='null', total_latency=0, model_stats={}, first_token_time='', requests_start_time='') raw_content=None
type='assistant' content='帮' name=None tool_calls=ToolCall(name='', args={}, id='') usage_metadata=UsageMetadata(code=0, errmsg='成功', prompt='null', task_id=None, model_name='Qwen/QwQ-32B', finish_reason='null', total_latency=0, model_stats={}, first_token_time='', requests_start_time='') raw_content=None
type='assistant' content='你的吗？' name=None tool_calls=ToolCall(name='', args={}, id='') usage_metadata=UsageMetadata(code=0, errmsg='成功', prompt='null', task_id=None, model_name='Qwen/QwQ-32B', finish_reason='null', total_latency=0, model_stats={}, first_token_time='', requests_start_time='') raw_content=None
type='assistant' content='' name=None tool_calls=ToolCall(name='', args={}, id='') usage_metadata=UsageMetadata(code=0, errmsg='成功', prompt='null', task_id=None, model_name='Qwen/QwQ-32B', finish_reason='null', total_latency=0, model_stats={}, first_token_time='', requests_start_time='') raw_content=None
type='assistant' content='你好！有什么我可以帮你的吗？' name=None tool_calls=ToolCall(name='', args={}, id='') usage_metadata=UsageMetadata(code=0, errmsg='成功', prompt='null', task_id=None, model_name='Qwen/QwQ-32B', finish_reason='stop', total_latency=0, model_stats={}, first_token_time='', requests_start_time='') raw_content=None
```
