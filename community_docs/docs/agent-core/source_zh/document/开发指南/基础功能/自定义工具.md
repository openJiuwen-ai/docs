...
```text
调用成功，返回结果: type='function' function=Function(name='add', description='本地加法插件', parameters=Parameters(type='object', properties={'a': {'description': '第一个参数', 'type': 'integer'}, 'required': ['a', 'b'], 'b': {'description': '第二个参数', 'type': 'integer'}}, required=['a', 'b']))
调用成功，返回结果: 6
```
