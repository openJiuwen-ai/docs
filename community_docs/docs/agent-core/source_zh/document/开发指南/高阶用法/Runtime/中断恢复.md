...
flow.set_end_comp("end", End(),
                  inputs_schema={
                      "result": "${interactive_node.result}"})
...
其中`output`为工作流输出，详细见[工作流输出介绍]()；`result`为列表，列表中每个元素为流式输出的元素，详细见[流式输出介绍]()；`type`为`__interaction__`代表消息类型为中断消息，`index`为`0`代表组件的第一个中断，`payload`中的内容为中断输出，详细见[中断输出介绍]()。
...
```python
{'responseContent': '', 'output': {'result': 'Yes'}}
```
...
flow.set_end_comp("end", End(),
                  inputs_schema={
                      "result1": "${interactive_node1.result}", "result2": "${interactive_node2.result}"})
...
其中`output`为工作流输出，详细见[工作流输出介绍]()；`result`为列表，列表中每个元素为流式输出的元素，详细见[流式输出介绍]()；`type`为`__interaction__`代表消息类型为中断消息，`index`为`0`代表每个组件的第一个中断，`payload`中的内容为中断输出，详细见[中断输出介绍]()。
...
```python
{'responseContent': '', 'output': {'result1': 'Yes', 'result2': 'No'}}
```
