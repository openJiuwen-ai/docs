...
- **type**(str)：流式数据的类型。当前支持两种内置的流式消息类型：
  ...
  - `'end node stream'`：表示流式消息为[End]()组件的流式消息。
- **index**(int)：流式数据的下标。
- **payload**(Any)：流式数据的数据内容。

**样例**：

基于[​BaseStreamMode.OUTPUT]()样例获取到输出：
...
基于[BaseStreamMode.CUSTOM]()样例获取到输出：
...
- 样例一：基于[BaseStreamMode.TRACE]()的基础工作流样例获取到输出。
...
- 样例二：基于[BaseStreamMode.TRACE]()的循环工作流样例获取到输出。
...
- 样例三：基于[BaseStreamMode.TRACE]()的嵌套工作流获取到输出。
