...
* 当某组件A与后续组件**存在**流式连接，与前序组件也**存在**流式连接时，组件A执行时调用`invoke`方法、`stream`方法和`tranform`方法。
...
>>> from openjiuwen.core.workflow.base import WorkflowOutput, Workflow
>>>
...
...         super().__init__()
... 
...     async def invoke(self, inputs, runtime: Runtime, context: Context) -> WorkflowOutput:
...         a = int(inputs.get('a'))
...         b = int(inputs.get('b'))
...         return {'result' : a + b}
... 
...
```python
async collect(self, inputs: AsyncIterator[Input], runtime: Runtime, context: Context)->Output
```
...
...
>>> print(result.result)
{'responseContent': '', 'output': {'results': [{'a': 1, 'b': 2, 'op': 'add', 'result': 3}, {'a': 2, 'b': 3, 'op': 'add', 'result': 5}, {'a': 4, 'b': 5, 'op': 'add', 'result': 9}]}}
```

### transform

```python
async transform(self, inputs: AsyncIterator[Input], runtime: Runtime, context: Context)-> AsyncIterator[Output]
```
...
>>> from openjiuwen.core.workflow.workflow_config import ComponentAbility
>>> 
>>> # 1. 自定义`TranformDemoComponent`组件，并实现了`transform`方法，用于多轮加法计算
