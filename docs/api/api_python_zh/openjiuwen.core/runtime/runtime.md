...
输出openJiuwen标准格式[OutputSchema]()流式消息，并流出到Agent或者Workflow外。

**参数**：

- **data** (Union[dict, OutputSchema])：流式消息。该消息有严格的格式规定。
  - 直接提供`OutputSchema`对象，[OutputSchema]()为openJiuwen内置的标准流式输出格式。
  - 可以提供`dict`类型的字段，但是必须包含[OutputSchema]()中定义的字段`'type'`、`'index'`、`'payload'`，并且数据类型相同。
...
输出自定义格式[CustomSchema]()的流式消息，并流出到Agent或者Workflow外。
...
type='workflow_final' index=0 payload={'responseContent': '', 'output': None}
```
...
获取当前Agent的config信息，若不存在，则返回`None`。Agent的config可参考 [ReActAgentConfig]()和[WorkflowAgentConfig]()。
...
...
{'responseContent': '', 'output': {'profile': {'agent_profile': 'demo_agent-demo_version-this_is_a_demo'}}}
```
...
获取指定工作流的config信息，若不存在，则返回`None`。工作流的config可参考[WorkflowConfig]()。
...
...     print(asyncio.get_event_loop().run_until_complete(run_workflow()))
...
{'responseContent': '', 'output': {'profile': {'agent_profile': 'fick_agent-fick_version-this_is_a_demo', 'workflow_profile': 'fick_name-fick_version-this_is_a_demo', 'execution_id': 'com', 'session_id': '12345'}}}
...
输出运行信息的流式消息，并以[TraceSchema]()格式的流式消息流出到Agent或者Workflow外，方便用户深入了解内部的运行机制，确保流程按照预期进行，同时能够快速找到问题所在，提高工作效率。

**参数**：

- **data**(dict)：需要溯源记录的运行数据。该数据将会填充到openJiuwen内置的流式消息格式[TraceSchema]()中payload结构中的`onInvokeData`字段，当为`None`时，表示没有运行数据。
...
...         if 'onInvokeData' in chunk.payload:
...
输出运行过程中的错误信息，并以[TraceSchema]()格式的流式消息输出到Workflow或者Agent外。

**参数**：

- **error**(Exception)：错误异常信息。该数据将会填充到openJiuwen内置的流式消息格式[TraceSchema]()中payload结构中的`error`字段。
...
...     if 'error' in chunk.payload and chunk.payload["error"] is not None and chunk.payload["output"] is None:
...
中断当前Agent或者Workflow执行，与用户进行交互，并等待用户补充输入[InteractionInput]()后，从中断位置继续执行。该接口目前仅支持在组件的非流式输入模式下的`invoke`和`stream`接口中使用。
...
**Any**，用户补充输入[InteractionInput]()中的`raw_inputs`或者`value`。
...
```python
get_workflow(self, workflow_id: str) -> Optional[Workflow]
```
...
>>> print(runtime.get_model("model").timeout)
10
```
...
>>> print(runtime.get_model('model_1').timeout)
1
>>> print(runtime.get_model('model_2').timeout)
2
```
...
...         return ModelFactory().get_model(model_provider="siliconflow", api_key="your path model service", api_base="sk-****************************", max_retrie=1, timeout=timeout)
... 
...     async def invoke(self, inputs: Input, runtime: Runtime, context: Context) -> Output:
...         model = self._get_model(timeout=10)
...         runtime.add_model("model", model)
