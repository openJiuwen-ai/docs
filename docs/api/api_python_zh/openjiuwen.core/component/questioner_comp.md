...
```python
>>> fildinfo = FieldInfo(field_name="date", description="时间", required=True, default_value="today")
```
...
- **model**​([ModelConfig]([Wiki: ModelConfig](../common/configs/model_config.md#class-openjiuwencorecomponentcommonconfigsmodel_configmodelconfig)))：大模型的配置信息。注意：未配置大模型、或配置大模型错误，组件将抛出异常。
...
- 输出：提问器组件的输出定义包括最近一次向用户追问的内容`question`、最近一次用户的反馈内容`user_response`以及以key-value键值对的形式输出提问器提取到的参数。具体如下：
  ...
  - **key-value键值对**：当参数提取的开关`extract_fields_from_response`打开，提问器提取到的参数，这些待提取参数的键在`field_names`中已经定义，提问器组件负责提取这些待提取参数的值；如果参数提取的开关关闭，则不存在参数提取的结果。
...
>>> workflow.set_end_comp("end", end_component, inputs_schema={"output": "${questioner.location}"})
...
>>> user_input.update(interrupted_component_id, "地点是杭州")  # 从上一次追问用户的信息中能够获取到工作流中的断点组件id，并且反馈用户结果
