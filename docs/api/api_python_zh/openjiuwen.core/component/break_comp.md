...
> **说明**
>
> * `BreakComponent`可与[LoopComponent](./llm_comp.md#class-openjiuwencorecomponentbasellmcomponent)和`loop_type="always_true"`（无限循环）配合使用。通过设置无限循环条件，使循环持续执行，直到遇到中断组件时才会退出循环。
...
...     # 第二个分支条件：当当前循环索引小于(循环次数-1)时，没有特定要执行的组件，回到循环起始点继续下一次循环
...     sw.add_branch("${l.index} < ${loop_number} - 1", ["s"], "2")
...
...
{'responseContent': '', 'output': {'array_result': [10, 11], 'user_var': 22}
```
