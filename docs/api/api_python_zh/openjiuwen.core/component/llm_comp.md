...
* **enable_history**(bool, 可选)：表示调用大模型的输入信息中，是否包含对话历史，`True`表示不使用对话历史，`False`表示不使用对话历史信息。默认值：`False`。
...
## class openjiuwen.core.component.base.LLMComponent

```python
class openjiuwen.core.component.base.LLMComponent(component_config: Optional[LLMCompConfig] = None)
```
...
>>> workflow.set_end_comp("end", end_component, inputs_schema={"output": "${llm.output}"})
>>> workflow.add_workflow_comp("llm", llm_comp, inputs_schema={"query": "${start.query}"})
>>> workflow.add_connection("start", "llm")
>>> workflow.add_connection("llm", "end")
>>> 
>>> result = await workflow.invoke({"query": "生成一句关于月亮的诗句", "conversation_id": "c123"}, WorkflowRuntime())
