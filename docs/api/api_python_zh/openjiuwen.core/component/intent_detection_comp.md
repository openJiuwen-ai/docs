...
```python
add_branch(condition: str, target: Union[str, list[str]], branch_id: str = None)
```
...
- **condition**(str)：条件分支的表达式。注意：表达式中需要包含意图识别组件在工作流中的id，`classification_id`对应意图识别组件的输出字段，`1`对应用户自定义的首个意图，后续依次递增。
...
>>> 
>>> intent_component.add_branch("${intent.classification_id} == 1", ["llm"], "查询景点分支")
>>> intent_component.add_branch("${intent.classification_id} == 2", ["llm"], "查询天气分支")
...
>>> workflow.set_end_comp("end", end_component, inputs_schema={"output": "${intent.category_name}"})
>>> workflowflow.add_workflow_comp(
...             "intent",
...             intent_component,
