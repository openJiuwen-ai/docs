...
... ]
>>> agent_context_impl.batch_add_messages(dict_messages, tags={"session": "weather_query"})
>>>
```
...
### get_latest_message
```python
get_latest_message(role: str = None) -> Optional[BaseMessage]
```
...
...     {"role": "assistant", "content": "今天天气晴朗，温度25度。"}
... ]
>>> workflow_context_impl.batch_add_messages(dict_messages, tags={"session": "weather_query"})
```
