...
```python
class openjiuwen.core.context_engine.engine.ContextEngine(agent_id:str, config: ContextEngineConfig = None, model: Optional[BaseChatModel] = None)
```
`ContextEngine`类用于管理模型输入上下文的信息，包含对上下文信息（长短期记忆、工具、知识等）的记录、获取、筛选等，便于开发Agent时对上下文内容进行标准化管理和调用。

**参数**：

- **agent_id**(str)：当前agent id，默认值：`""`。
- **config**([ContextEngineConfig](), 可选)：上下文引擎相关配置参数。默认值：`None`，表示无可用配置。
- **model**([BaseChatModel](), 可选)：大模型实例。默认值：`None`，表示没有大模型实列。
...
...     # 通过workflow_id和session_id获取工作流中上下文引擎实例
