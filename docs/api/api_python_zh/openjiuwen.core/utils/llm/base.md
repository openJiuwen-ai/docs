...
* **tools**​(Union[List[[ToolInfo](messages.md#class-openjiuwencoreutilsllmmessagestoolinfo)], List[Dict]], 可选)​：指定模型可调用的工具列表，默认值：`None`，表示没有可选工具。
...
... )
... 
>>> # 方式二：使用别名（推荐：与多数供应商字段保持一致）
>>> info2 = BaseModelInfo(
...     api_key="sk-xxx",
...     api_base="your path to model service",
...     model="qwen-max",          # <-- 别名等价于 model_name
...     stream=False,              # <-- 别名等价于 streaming
... )
... 
>>> print(info1.model_name, info1.streaming)
>>> print(info2.model_name, info2.streaming)
qwen-plus True
qwen-max False
```
