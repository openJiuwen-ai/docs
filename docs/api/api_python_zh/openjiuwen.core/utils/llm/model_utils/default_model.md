...
```python
_invoke(self, model_name: str, messages: List[Dict], tools: List[Dict] = None, temperature: float = 0.1, top_p: float = 0.1, **kwargs) -> AIMessage
```
...
* ​​**tools**​(List[Dict], 可选)​：指定模型可调用的工具列表，参数构造可参考[ToolInfo](../messages.md#class-openjiuwencoreutilsllmmessagestoolinfo)。默认值：`None`，表示没有可选工具。
...
```python
async _ainvoke(self, model_name: str, messages: List[Dict], tools: List[Dict] = None, temperature: float = 0.1, top_p: float = 0.1, **kwargs) -> AIMessage
```
...
```python
_stream(self, model_name: str, messages: List[Dict], tools: List[Dict] = None, temperature: float = 0.1, top_p: float = 0.1, **kwargs) -> Iterator[AIMessageChunk]
```
...
```python
async  _astream(self, model_name: str, messages: List[Dict], tools: List[Dict] = None, temperature: float = 0.1, top_p: float = 0.1, **kwargs) -> AsyncIterator[AIMessageChunk]
```
...
```python
_invoke(model_name:str, messages: List[Dict], tools: List[Dict] = None, temperature:float = 0.1, top_p:float = 0.1, **kwargs: Any) -> AIMessage
```
...
```python
async _ainvoke(model_name:str, messages:List, tools:List=None, temperature:float=0.1, top_p:float=0.1, **kwargs:Any) -> AIMessage
```
...
```python
_stream(model_name:str, messages:List, tools:List=None, temperature:float=0.1, top_p:float=0.1, **kwargs:Any) -> Iterator[AIMessageChunk]
```
...
```python
async _astream(model_name:str, messages:List, tools:List=None, temperature:float=0.1, top_p:float=0.1, **kwargs:Any) -> AsyncIterator[AIMessageChunk]
```
