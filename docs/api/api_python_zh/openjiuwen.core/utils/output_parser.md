# openjiuwen.core.utils.output_parser.base.BaseOutputParser

## class openjiuwen.core.utils.output_parser.base.BaseOutputParser
...
# openjiuwen.core.utils.output_parser.base.JsonOutputParser

## class openjiuwen.core.utils.output_parser.base.JsonOutputParser

```python
openjiuwen.core.utils.output_parser.base.JsonOutputParser(BaseOutputParser)
```
...
```python
async parse(self, inputs: Union[str, AIMessage]) -> Dict
```
...
- **inputs**(Union[str, AIMessage])：单次请求的完整输入，可以是模型返回的字符串或`AIMessage`对象。

**返回**：

**Dict**，字典数据类型，表示解析后的JSON数据。
...
```python
async stream_parse(self, streaming_inputs: Union[Iterator[str], Iterator[AIMessageChunk]]) -> Iterator[Dict]
```
...
- **streaming_inputs**(Union[Iterator[str], Iterator[AIMessageChunk]])：单次请求的完整输入，输入是一个迭代器，包含多个数据块，可以是字符串或`AIMessageChunk`对象，每个数据块是从流式响应中读取的部分内容，方法会逐步将它们合并为完整的 JSON 数据进行解析。

**返回**：

**Iterator[Dict]**，返回一个迭代器，每次迭代返回一个解析后的字典对象。
...
# openjiuwen.core.utils.output_parser.markdown_output_parser.MarkdownOutputParser

## class openjiuwen.core.utils.output_parser.markdown_output_parser.MarkdownOutputParser

```python
openjiuwen.core.utils.output_parser.markdown_output_parser.MarkdownOutputParser(BaseOutputParser)
```
...
>>> import asyncio
>>> 
>>> from openjiuwen.core.utils.output_parser.markdown_output_parser import MarkdownOutputParser
>>> 
>>> async def main():
...
- **streaming_inputs**(Union[Iterator[str], Iterator[AIMessageChunk]])：LLM 输出的迭代器，可以是字符串块或`AIMessageChunk`对象。
