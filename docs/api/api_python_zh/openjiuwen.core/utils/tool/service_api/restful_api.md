...
* **method** (str)：API接口的请求方法。取值可为`GET`、`POST`、`PUT`或`DELETE`。
...
调用RESTful API接口的方法，用于发送请求并获取接口响应结果。
...
... }
... 
>>> result = weather_api.invoke(inputs=inputs)
...
... }
... 
>>> result = await weather_api.ainvoke(inputs=inputs)
>>> print(f"调用成功，返回结果: {result}")
调用成功，返回结果: beijing 2023-10-01的天气晴朗
```
