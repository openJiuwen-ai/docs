...
```python
openjiuwen.agent.config.base.AgentConfig(id: str = "", version: str = "", description: str = "", controller_type: ControllerType = ControllerType.Undefined, plugins: List[PluginSchema] = [], workflows: List[WorkflowSchema] = [])
```
...
- **plugins**(List[[PluginSchema](../common/schema.md#class-openjiuwenagentcommonschemapluginschema)], 可选)：agent使用的插件描述信息列表。默认值：[]，表示没有可调用的插件。
...
... )
>>> 
>>> print(f"插件数量: {len(agent_config_full.plugins)}")
插件数量: 1
